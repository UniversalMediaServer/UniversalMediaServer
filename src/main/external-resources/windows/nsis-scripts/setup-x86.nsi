...
Section "Uninstall"
	...
	Delete /REBOOTOK "$INSTDIR\renderers\Sony-X-Series-TV-2018-and-earlier.conf"
	Delete /REBOOTOK "$INSTDIR\renderers\Sony-X-Series-TV-2019-and-later.conf"
...
Function .onInit
	${If} ${RunningX64}
		MessageBox MB_YESNO|MB_ICONQUESTION|MB_DEFBUTTON2 'There is a specific 64 bit installer version$\r$\nProceed with old installer anyway?' IDYES goanyway
        Quit
        goanyway:
	${EndIf}
	SetShellVarContext all
