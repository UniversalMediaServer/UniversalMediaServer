...
public abstract class DLNAResource extends HTTPResource implements Cloneable, Runnable {
	...
	public final String getDidlString(RendererConfiguration mediaRenderer) {
		...
		if (!configuration.isDisableSubtitles() && mediaRenderer.getSupportedSubtitles() != null && getMedia() != null && getPlayer() == null) {
			...
			if (getMediaSubtitle() != null) {
				...
				for (int i = 0; i < params.length; i++) {
					if (getMediaSubtitle().getType().toString().equals(params[i])) {
						subsAreValid = true;
						break;
					}
				}
		...
		if (firstAudioTrack != null && isNotBlank(firstAudioTrack.getSongname())) {
			...
			// Ditlew
			wireshark = ((isFolder() || getPlayer() == null && subsAreValid) ? getDisplayName() : mediaRenderer.getUseSameExtension(getDisplayName(mediaRenderer)));
		...
		if (!isFolder()) {
			...
			for (int c = 0; c < indexCount; c++) {
				...
				if (subsAreValid) {
					...
					wireshark = wireshark + " " + "pv:subtitleFileType=" + getMediaSubtitle().getType().toString();
					addAttribute(sb, "pv:subtitleFileUri", getSubsURL(getMediaSubtitle()));
					wireshark = wireshark + " " + "pv:subtitleFileUri=" + getSubsURL(getMediaSubtitle());
				}
	...
	/**
	 * This code block comes from Player.setAudioAndSubs()
	 * TODO try to make it in Player itself
	 */
	private void setExternalSubs() {
		// Add subtitles for renderers capable to play external subtitles
		String currentLang = null;
		DLNAMediaSubtitle matchedSub = null;
		OutputParams params = new OutputParams(configuration);
		boolean finishedMatchingPreferences = false;
