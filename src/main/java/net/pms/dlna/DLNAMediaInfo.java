					...
					try {
						...
						if (ah != null && !thumbOnly) {
							...
							if (ah.getChannels() != null && ah.getChannels().toLowerCase().contains("mono")) {
								...
								try {
									int channels = Integer.parseInt(ah.getChannels());
									if (channels > 0) {
										audio.getAudioProperties().setNumberOfChannels(channels);
									} else {
										LOGGER.warn("Invalid number of audio channels \"{}\" for file: {}", ah.getChannels(), af.getFile().getName());
									}
								} catch (NumberFormatException e) {
									LOGGER.debug("Couldn't parse the number audio channels \"{}\" for file: \"{}\"", ah.getChannels(), af.getFile().getName());
								}
							}
