 ...
 * {@link BufferedInputStream} if you want to alter the buffering behavior. This
 * will fake the behavior of a {@link BufferedInputStream} with its own "mark
 * position" while keeping the real mark at 0. This means that the start
 ...
 * <br>
 * The intended use for this class is to use an {@link InputStream} instance for
 * multiple purposes without having to read the whole stream into memory, for
 * example when parsing information early in the stream with multiple parsers.
 *
 * @author Nadahar
 */
public class ResettableInputStream extends BufferedInputStream {
	...
	/**
	 * Creates a {@link ResettableInputStream} and saves its argument, the input
	 * stream {@code in}, for later use. An internal buffer array is created and
	 * stored in {@code buf}.
	 *
	 * @param in the underlying input stream.
	 * @param marklimit the maximum size to buffer before the mark/start point
	 *            gets invalidated. This overrides the parameter sent when
	...
	/**
	 * Creates a {@link ResettableInputStream} with the specified buffer size,
	 * and saves its argument, the input stream {@code in}, for later use. An
	 * internal buffer array of length {@code size} is created and stored in
	 * {@code buf}.
	 *
	 * @param in the underlying input stream.
	 ...
	 * invalidated), an {@link IOException} is thrown. Otherwise, {@code pos} is
