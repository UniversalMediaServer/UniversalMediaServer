...
public class GeneralTab {
	...
	public JComponent build() {
		...
		if (!configuration.isHideAdvancedOptions()) {
			...
			boolean disableSleepSupported = PMS.get().getRegistry().disableGoToSleepSupported();
			if (disableSleepSupported) {
			...
			builder.add(newHTTPEngine, FormLayoutUtil.flip(cc.xy(disableSleepSupported ? 7 : 1, ypos), colSpec, orientation));
			ypos += 2;
