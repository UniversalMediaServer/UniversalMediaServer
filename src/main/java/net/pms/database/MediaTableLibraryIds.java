...
import net.pms.library.LibraryId;
 ...
 */
public class MediaTableLibraryIds extends MediaTable {

	private static final Logger LOGGER = LoggerFactory.getLogger(MediaTableLibraryIds.class);
	public static final String TABLE_NAME = "LIBRARY_IDS";
	...
	/**
	 * Get an id for a LibraryResource.
	 *
	 * Usefull for the rootfolder to use the old known id if any.
	 *
	 * @param connection
	 * @param resource
	 * @return the LibraryId stored or created
	 */
	public static LibraryId getResourceLibraryId(Connection connection, LibraryResource resource) {
	 ...
	 */
	public static LibraryId getLibraryId(Connection connection, long id) {
		if (connection == null) {
			return null;
		}
		try (PreparedStatement stmt = connection.prepareStatement(SQL_GET_ALL_ID)) {
			...
			try (ResultSet elements = stmt.executeQuery()) {
				if (elements.next()) {
					LibraryId result = new LibraryId();
	...
	public static void setLibraryUpdateId(Connection connection, long id, long updateId) {
		if (connection == null) {
			return;
		}
		...
		try (PreparedStatement stmt = connection.prepareStatement(SQL_GET_ALL_ID, ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_UPDATABLE)) {
			...
			try (ResultSet elements = stmt.executeQuery()) {
				if (!elements.next()) {
					...
					elements.updateString(COL_OBJECT_TYPE, "RootFolder");
