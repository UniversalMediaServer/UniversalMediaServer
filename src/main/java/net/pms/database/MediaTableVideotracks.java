...
public class MediaTableVideotracks extends MediaTable {
	...
	public static final String SQL_GET_FILEID_BY_VIDEO4K = SELECT + TABLE_COL_FILEID + FROM + TABLE_NAME + WHERE + TABLE_COL_WIDTH + " > 3700" + OR + TABLE_COL_HEIGHT + " > 2000";
	...
	private static void createTable(final Connection connection) throws SQLException {
		...
		execute(connection,
			...
			")",
			CREATE_INDEX + IF_NOT_EXISTS + TABLE_NAME + CONSTRAINT_SEPARATOR + COL_WIDTH + CONSTRAINT_SEPARATOR + COL_HEIGHT + IDX_MARKER + ON + TABLE_NAME + "(" + COL_WIDTH + COMMA + COL_HEIGHT + ")",
			CREATE_INDEX + IF_NOT_EXISTS + TABLE_NAME + CONSTRAINT_SEPARATOR + COL_MULTIVIEW_LAYOUT + IDX_MARKER + ON + TABLE_NAME + "(" + COL_MULTIVIEW_LAYOUT + ")"
		);
	}
