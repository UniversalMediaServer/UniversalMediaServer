...
import net.pms.library.FeedItem;
import net.pms.library.LibraryResource;
import net.pms.library.RealFile;
import net.pms.library.WebAudioStream;
import net.pms.library.WebVideoStream;
...
public class UnattachedFolder extends VirtualFolder {
	...
	public LibraryResource add(LibraryResource d) {
		if (d != null) {
			addChild(d);
	...
	public LibraryResource add(String uri, String name) {
		LibraryResource d = autoMatch(uri, name);
		if (d != null) {
	...
	public LibraryResource get(String objectId) {
		int index = getIndex(objectId);
		return index > -1 ? getChildren().get(index) : null;
	}

	public List<LibraryResource> asList(String objectId) {
		int index = getIndex(objectId);
	...
	public LibraryResource recreate(String objectId, String name) {
		try {
	...
	private LibraryResource autoMatch(String uri, String name) {
		try {
		...
		LibraryResource resource = isweb ?
