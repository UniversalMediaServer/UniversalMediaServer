...
import net.pms.library.RarredFile;
import net.pms.library.RealFile;
import net.pms.library.SevenZipFile;
import net.pms.library.ZippedFile;
...
public class VirtualFile extends LibraryResource {
		...
		if (f.isFile() || f.isDirectory()) {
			...
			if (!f.isHidden()) {
				if (renderer.getUmsConfiguration().isArchiveBrowsing() && (lcFilename.endsWith(".zip") || lcFilename.endsWith(".cbz"))) {
					addChild(new ZippedFile(renderer, f), true, isAddGlobally);
				} else if (renderer.getUmsConfiguration().isArchiveBrowsing() && (lcFilename.endsWith(".rar") || lcFilename.endsWith(".cbr"))) {
					addChild(new RarredFile(renderer, f), true, isAddGlobally);
				} else if (
					renderer.getUmsConfiguration().isArchiveBrowsing() && (
					...
					addChild(new SevenZipFile(renderer, f), true, isAddGlobally);
				} else if (
					...
					addChild(new DVDISOFile(renderer, f), true, isAddGlobally);
				} else if (
					...
					LibraryResource d = PlaylistFolder.getPlaylist(renderer, lcFilename, f.getAbsolutePath(), 0);
					if (d != null) {
					...
					if (f.isDirectory() && renderer.getUmsConfiguration().isHideEmptyFolders() && !FileUtil.isFolderRelevant(f, renderer.getUmsConfiguration())) {
						...
						RealFile rf = new RealFile(renderer, f);
	...
	public boolean analyzeChildren(int count, boolean isAddGlobally) {
		...
		while (((getChildren().size() - currentChildrenCount) < count) || (count == -1)) {
			if (vfolder < virtualFolders.size()) {
				...
				parent.addChild(new VirtualFile(renderer, virtualFolder), true, isAddGlobally);
				++vfolder;
			} else {
