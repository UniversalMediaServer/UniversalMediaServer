...
public class DbIdResourceLocator {
					...
					switch (typeAndIdent.type) {
						case TYPE_AUDIO, TYPE_VIDEO, TYPE_IMAGE -> {
							...
							if (filename != null) {
								...
								res = renderer.getRootFolder().createResourceFromFile(file);
								res.resolve();
							}
						}
						case TYPE_PLAYLIST -> {
						...
						case TYPE_ALBUM -> {
							...
							try (ResultSet resultSet = statement.executeQuery(sql)) {
								...
								while (resultSet.next()) {
									LibraryResource item = createResourceFromFile(
											renderer,
	...
	public static LibraryResource createResourceFromFile(Renderer renderer, File file, DbIdTypeAndIdent typeAndIdent) {
		LibraryResource item = renderer.getRootFolder().createResourceFromFile(file);
