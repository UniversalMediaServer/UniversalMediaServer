...
import net.pms.library.container.ChapterFileTranscodeVirtualFolder;
 ...
 */
public abstract class LibraryItem extends LibraryResource {
	...
	protected LibraryItem(Renderer renderer) {
		this(renderer, Format.UNKNOWN);
	}

	protected LibraryItem(Renderer renderer, int specificType) {
		super(renderer);
	...
	/**
	 * Sets the LibraryResource's {@link net.pms.formats.Format} according to
	 ...
	 * of mediaInfo. Also returns true if this LibraryResource is a container.
	 */
	...
	/**
	 * Returns an InputStream of this LibraryResource that starts at a given
	...
	private LibraryResource resumeStop() {
		...
		if (resume != null) {
			...
			for (LibraryResource res : getParent().getChildren()) {
				if (res instanceof LibraryItem item && item.isResume() && item.getName().equals(getName())) {
