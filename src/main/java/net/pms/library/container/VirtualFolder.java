...
import net.pms.library.FileSearch;
import net.pms.library.LibraryContainer;
import net.pms.library.LibraryResource;
import net.pms.library.SystemFileResource;
import net.pms.library.SystemFilesHelper;
...
public class VirtualFolder extends LibraryContainer {
	...
	private ArrayList<LibraryResource> searchList;
	...
	private void manageFile(File f, boolean isAddGlobally) {
		LibraryResource res = renderer.getRootFolder().createResourceFromFile(f);
		if (res != null) {
	...
	public boolean analyzeChildren(int count, boolean isAddGlobally) {
		...
		while (((getChildren().size() - currentChildrenCount) < count) || (count == -1)) {
			if (vfolder < virtualFolders.size()) {
				...
				LibraryContainer parent = getSharedContentParent(virtualFolder.getParent());
