...
public class ITunesLibrary extends LibraryContainer {
		...
		if (Platform.isMac() || Platform.isWindows()) {
			...
			try {
				...
				if (iTunesFile != null && (new File(iTunesFile)).exists()) {
					...
					LibraryContainer playlistsFolder = null;
					...
					for (Object item : playlists) {
						...
						if (playlist.containsKey("Music") && playlist.get("Music").equals(Boolean.TRUE)) {
							...
							LibraryContainer musicFolder = new LibraryContainer(renderer, playlist.get("Name").toString(), null);
							res.addChild(musicFolder);

							LibraryContainer virtualFolderArtists = new LibraryContainer(renderer, Messages.getString("BrowseByArtist"), null);
							LibraryContainer virtualFolderAlbums = new LibraryContainer(renderer, Messages.getString("BrowseByAlbum"), null);
							LibraryContainer virtualFolderGenres = new LibraryContainer(renderer, Messages.getString("BrowseByGenre"), null);
							LibraryContainer virtualFolderAllTracks = new LibraryContainer(renderer, Messages.getString("AllAudioTracks"), null);
							...
							if (playlistTracks != null) {
								for (Object t : playlistTracks) {
									...
									if (
										...
										LibraryContainer individualArtistFolder = null;
										LibraryContainer individualArtistAllTracksFolder;
										LibraryContainer individualArtistAlbumFolder = null;

										for (LibraryResource artist : virtualFolderArtists.getChildren()) {
											if (areNamesEqual(artist.getName(), artistName)) {
												individualArtistFolder = (LibraryContainer) artist;
												for (LibraryResource album : individualArtistFolder.getChildren()) {
													if (areNamesEqual(album.getName(), albumName)) {
														individualArtistAlbumFolder = (LibraryContainer) album;
													}
												}
												break;
											}
										...
										if (individualArtistFolder == null) {
											individualArtistFolder = new LibraryContainer(renderer, artistName, null);
											virtualFolderArtists.addChild(individualArtistFolder);
											individualArtistAllTracksFolder = new LibraryContainer(renderer, Messages.getString("AllAudioTracks"), null);
											individualArtistFolder.addChild(individualArtistAllTracksFolder);
										} else {
											individualArtistAllTracksFolder = (LibraryContainer) individualArtistFolder.getChildren().get(0);
										}

										if (individualArtistAlbumFolder == null) {
											individualArtistAlbumFolder = new LibraryContainer(renderer, albumName, null);
										...
										LibraryContainer individualAlbumFolder = null;
										for (LibraryResource album : virtualFolderAlbums.getChildren()) {
											if (areNamesEqual(album.getName(), albumName)) {
												individualAlbumFolder = (LibraryContainer) album;
												break;
											}
										}
										if (individualAlbumFolder == null) {
											individualAlbumFolder = new LibraryContainer(renderer, albumName, null);
										...
										LibraryContainer individualGenreFolder = null;
										for (LibraryResource genre : virtualFolderGenres.getChildren()) {
											if (areNamesEqual(genre.getName(), genreName)) {
												individualGenreFolder = (LibraryContainer) genre;
												break;
											}
										}
										if (individualGenreFolder == null) {
											individualGenreFolder = new LibraryContainer(renderer, genreName, null);
							...
							Collections.sort(virtualFolderArtists.getChildren(), (LibraryResource o1, LibraryResource o2) -> {
								LibraryContainer a = (LibraryContainer) o1;
								LibraryContainer b = (LibraryContainer) o2;
							...
							// Add all playlists
							LibraryContainer pf = new LibraryContainer(renderer, playlist.get("Name").toString(), null);
							playlistTracks = (List<?>) playlist.get("Playlist Items"); // list of tracks in a playlist
							...
							if (kind >= 0 && kind != 17 && kind != 19 && kind != 20) {
								...
								if (playlistsFolder == null) {
									playlistsFolder = new LibraryContainer(renderer, "Playlists", null);
