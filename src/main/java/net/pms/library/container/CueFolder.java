...
public class CueFolder extends LibraryContainer {
		...
		if (playlistfile.length() < 10000000) {
			...
			if (sheet != null) {
				...
				if (!files.isEmpty()) {
					...
					ArrayList<LibraryResource> addedResources = new ArrayList<>();
					for (int i = 0; i < tracks.size(); i++) {
						...
						if (i > 0) {
							...
							LibraryResource prec = addedResources.get(i - 1);
							int count = 0;
					...
					if (!tracks.isEmpty() && !addedResources.isEmpty()) {
						LibraryResource lastTrack = addedResources.get(addedResources.size() - 1);
