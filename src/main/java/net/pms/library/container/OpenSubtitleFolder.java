...
public class OpenSubtitleFolder extends LibraryContainer {
	private static final Logger LOGGER = LoggerFactory.getLogger(OpenSubtitleFolder.class);
	private final LibraryItem originalResource;

	public OpenSubtitleFolder(Renderer renderer, LibraryItem resource) {
	...
	public void discoverChildren() {
		try {
			List<SubtitleItem> subtitleItems = OpenSubtitle.findSubtitles(originalResource, renderer);
			if (subtitleItems == null || subtitleItems.isEmpty()) {
				return;
			}
			Collections.sort(subtitleItems, new SubSort(renderer));
			reduceSubtitles(subtitleItems, renderer.getUmsConfiguration().getLiveSubtitlesLimit());
			LOGGER.trace(
			...
			for (SubtitleItem subtitleItem : subtitleItems) {
				...
				LibraryItem liveResource = originalResource.clone();
	...
	//TODO: (Nad) Keep hash results
	private static void reduceSubtitles(List<SubtitleItem> subtitles, int limit) {
