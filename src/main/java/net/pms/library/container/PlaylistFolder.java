...
public final class PlaylistFolder extends LibraryContainer {
	...
	protected void resolveOnce() {
		...
		for (Entry entry : entries) {
			...
			if (!isweb && !FileUtil.isUrl(entry.fileName)) {
				...
				LibraryResource d = type == Format.VIDEO ? new WebVideoStream(renderer, entry.title, u, null) :
		...
		for (LibraryResource r : getChildren()) {
			r.syncResolve();
		}
	}
	...
	public static LibraryContainer getPlaylist(Renderer renderer, String name, String uri, int type) {
