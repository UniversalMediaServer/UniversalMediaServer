...
public class ApertureLibraries extends LibraryContainer {
			...
			try {
				...
				try (BufferedReader in = new BufferedReader(new InputStreamReader(process.getInputStream()))) {
					...
					while ((line = in.readLine()) != null) {
						...
						LibraryContainer apertureLibrary = createApertureDlnaLibrary(renderer, line);
	...
	private static LibraryContainer createApertureDlnaLibrary(Renderer renderer, String url) throws XmlParseException, IOException, URISyntaxException {
		LibraryContainer res = null;
		...
		if (url != null) {
			...
			res = new LibraryContainer(renderer, mediaName, null);
			...
			for (Object item : listOfAlbums) {
				...
				if (album.get("Parent") == null) {
					LibraryContainer vAlbum = createApertureAlbum(renderer, photoList, album, listOfAlbums);
					res.addChild(vAlbum);
				}
			}
		} else {
	...
	private static LibraryContainer createApertureAlbum(
		Renderer renderer,
		Map<?, ?> photoList,
		...
		LibraryContainer vAlbum = new LibraryContainer(renderer, album.get("AlbumName").toString(), null);
		...
		for (Object item : listOfAlbums) {
			...
			if (sub.get("Parent") != null) {
				...
				if (parent == albumId) {
					LibraryContainer subAlbum = createApertureAlbum(renderer, photoList, sub, listOfAlbums);
