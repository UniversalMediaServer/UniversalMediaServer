...
import net.pms.library.LibraryItemSorter;
...
public class FileTranscodeVirtualFolder extends TranscodeVirtualFolder {
	...
	private final LibraryItem originalResource;

	public FileTranscodeVirtualFolder(Renderer renderer, LibraryItem resource) {
	...
	private static boolean isSeekable(LibraryItem resource) {
		Engine engine = resource.getEngine();

		return (engine == null) || engine.isTimeSeekable();
	}

	private void addChapterFolder(LibraryItem resource) {
	...
	protected void resolveOnce() {
		if (getChildren().isEmpty()) { // OK
			...
			ArrayList<LibraryItem> entries = new ArrayList<>();
			...
			LibraryItem noTranscode = createResourceWithAudioSubtitleEngine(originalResource, singleAudioTrack, null, null);
			...
			for (MediaAudio audio : audioTracks) {
				...
				for (MediaSubtitle subtitle : subtitlesTracks) {
					...
					LibraryItem temp = createResourceWithAudioSubtitleEngine(originalResource, audio, subtitle, null);
					...
					for (Engine engine : engines) {
						LibraryItem copy = createResourceWithAudioSubtitleEngine(originalResource, audio, subtitle, engine);
						entries.add(copy);
					}
				}
			}
			...
			if (renderer.isSubtitlesStreamingSupportedForAllFiletypes()) {
				...
				for (MediaSubtitle subtitlesTrack : subtitlesTracks) {
					if (subtitlesTrack != null && subtitlesTrack.isExternal() &&
						...
						LibraryItem copy = createResourceWithAudioSubtitleEngine(originalResource, singleAudioTrack, subtitlesTrack, null);
						entries.add(copy);
					}
			...
			for (LibraryItem resource : entries) {
