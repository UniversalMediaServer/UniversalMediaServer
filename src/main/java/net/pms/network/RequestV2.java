...
public class RequestV2 extends HTTPResource {
	...
	public ChannelFuture answer(
		...
		if ((method.equals("GET") || method.equals("HEAD")) && argument.startsWith("console/")) {
			...
			if (soapaction != null && soapaction.contains("ContentDirectory:1#GetSystemUpdateID")) {
				...
				if (LOGGER.isTraceEnabled()) {
					...
					if (event.getMessage() instanceof DefaultHttpRequest) {
						sb.append(" ").append(((DefaultHttpRequest) event.getMessage()).getUri());
					}
					if (event.getMessage() instanceof HttpMessage) {
						sb.append(" ").append(((HttpMessage) event.getMessage()).getProtocolVersion().getText());
					}
					if (event.getMessage() instanceof DefaultHttpMessage) {
						sb.append(" Chunked: ").append(((DefaultHttpMessage) event.getMessage()).isChunked());
					}
					sb.append("\n");
					...
					if (event.getMessage() instanceof HttpMessage) {
						...
						sb.append("Unknown class: ").append(event.getClass().getSimpleName()).append("\n");
						sb.append(event);
					}
					...
					LOGGER.trace(
						...
						soapaction.contains("ContentDirectory:1#Browse") ? "browse" : "search",
						mediaRenderer.getConfName(),
						sb.toString(),
						isNotBlank(formattedContent) ? "CONTENT:\n" + formattedContent : "NO CONTENT"
					);
				}
		...
		if (LOGGER.isTraceEnabled()) {
			...
			if (method.equals("HEAD")) {
				LOGGER.trace("HEAD only response sent to {} with {} and status \"{}\":\n\nHEADER:\n{}",
					mediaRenderer.getConfName(),
				...
				if (isNotBlank(formattedResponse)) {
					LOGGER.trace(
						"Response sent to {} with {} and status \"{}\":\n\nHEADER:\n{}\nCONTENT:\n{}",
						mediaRenderer.getConfName(),
						output.getProtocolVersion(),
						output.getStatus(),
						header.toString(),
					...
					LOGGER.trace(
						"{} transfer response sent to {} with {} and status \"{}\":\n\nHEADER:\n{}",
					...
					LOGGER.trace(
						"Empty response sent to {} with {} and status \"{}\":\nHEADER:\n{}",
						mediaRenderer.getConfName(),
						output.getProtocolVersion(),
						output.getStatus(),
						header.toString()
