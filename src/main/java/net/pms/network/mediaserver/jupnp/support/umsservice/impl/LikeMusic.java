...
public class LikeMusic {
	...
	/**
	 * Album likes live in RESOURCE_RATINGS now. They are written back into the
	 * legacy like tables first, so the backup file format stays unchanged and
	 * older backups remain restorable.
	 */
	...
	public void restoreLikedAlbums() throws SQLException, FileNotFoundException {
		...
		//the restored legacy tables are only a transport format, the likes have to
		//end up in RESOURCE_RATINGS to show up in My Albums
		try (Connection connection = MediaDatabase.getConnectionIfAvailable()) {
