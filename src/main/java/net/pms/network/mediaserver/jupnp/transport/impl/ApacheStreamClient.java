 ...
 */
public class ApacheStreamClient extends AbstractStreamClient<ApacheStreamClientConfiguration, HttpUriRequest> {

	//base the logger inside org.jupnp.transport.spi.AbstractStreamClient to reflect old behavior
	...
	protected HttpUriRequest createRequest(StreamRequestMessage requestMessage) {
		...
		LOGGER.debug("Creating HTTP request. URI: '{}' method: '{}'", upnpRequest.getURI(), upnpRequest.getMethod());
	...
	protected Callable<StreamResponseMessage> createCallable(final StreamRequestMessage requestMessage, HttpUriRequest request) {
		return () -> {
			...
			try {
				...
				LOGGER.debug("Received HTTP response: {}", httpResponse.getStatusLine().getReasonPhrase());
				...
				if (bytes == null || 0 == bytes.length) {
					LOGGER.debug("HTTP response message has no entity");
					return responseMessage;
				}

				if (responseMessage.isContentTypeMissingOrText()) {
					LOGGER.debug("HTTP response message contains text entity");
				} else {
					LOGGER.debug("HTTP response message contains binary entity");
				}
				...
				if (e instanceof NoHttpResponseException) {
					LOGGER.debug("Request: {} failed : {}", request, e.getMessage());
	...
	protected boolean logExecutionException(Throwable t) {
		if (t instanceof IllegalStateException) {
			...
			LOGGER.debug("Illegal state: {}", t.getMessage());
			return true;
	...
	@Override
	public void stop() {
		LOGGER.debug("Shutting down HTTP client connection manager/pool");
		try {
			httpClient.close();
		} catch (IOException e) {
			LOGGER.trace("Shutting down of HTTP client throwed exception : {0}", e);
		}
	}
	...
	protected <O extends UpnpOperation> HttpEntity createEntityProvider(final UpnpMessage<O> upnpMessage) {
		if (upnpMessage.getBodyType().equals(UpnpMessage.BodyType.STRING)) {
			LOGGER.debug("Preparing HTTP request entity as String");
			...
			LOGGER.debug("Preparing HTTP request entity as byte[]");
