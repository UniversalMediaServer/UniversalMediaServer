...
public class PlayerApiServletTest {
	...
	public void testGetMediaLibraryFolderChildsForVideoFolderChildrenEmpty()throws Exception {
        ...
        Method method = PlayerApiServlet.class.getDeclaredMethod("getMediaLibraryFolderChilds",DLNAResource.class, Renderer.class, String.class);
        method.setAccessible(true);
         
        try {
			Mockito.when(videoFolder.getId()).thenReturn("100");
			JsonArray jLibraryVideos = (JsonArray) method.invoke(playerApiServletInstance,videoFolder,renderer,"100");
	  ...
	  DLNAResource abstractClass = Mockito.mock(DLNAResource.class,Mockito.CALLS_REAL_METHODS); // Use reflection to set the value of
	  ...
	  try { 
		  ...
		  Method method = PlayerApiServlet.class.getDeclaredMethod("getMediaLibraryFolderChilds",
		  DLNAResource.class, Renderer.class, String.class);
		  method.setAccessible(true);
		  Mockito.when(abstractClass.getDLNAResources(abstractClass.getId(), true, 0,0, renderer, "100")).thenReturn(libraryVideos);
		  JsonArray jLibraryVideos = (JsonArray)method.invoke(playerApiServletInstance,abstractClass,renderer,"100");
		  assertEquals(null, jLibraryVideos); 
	  }
	  catch (Exception ex) {
