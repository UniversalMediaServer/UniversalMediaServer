...
class FFmpegHlsSubtitlesTest {
	...
	private final FFmpegHlsVideo engine =
			new FFmpegHlsVideo() {
				@Override
	...
	void setup() throws Exception {
		...
		renderer =
				new Renderer((String) null) {
					...
					@Override
					public boolean isExternalSubtitlesFormatSupported(
							MediaSubtitle sub, net.pms.store.StoreItem resource) {
						return true;
					}
				};
		...
		item.setTranscodingSettings(
				new TranscodingSettings(engine, EncodingFormat.VIDEO.get("HLS-MPEGTS-H264-AAC")));
		params = new OutputParams(config);
	...
	private List<String> command() throws Exception {
		return engine.buildCommand(item, item.getMediaInfo(), params);
	}

	@Test
	void burnsSelectedExternalTrackWithConfiguredStyleAndPreservesSeekTimestamps() throws Exception {
		var sub = subtitle(100);
		sub.setExternalFileOnly(new File("selected.srt"));
	...
	@Test
	void burnsSecondEmbeddedTrackInsteadOfFirst() throws Exception {
	...
	@Test
	void offSelectionDoesNotExposeEmbeddedSubtitles() throws Exception {
	...
	@Test
	void ordinaryHlsKeepsSelectableSubtitleRenditions() throws Exception {
		item.setParent(null);
		item.setMediaSubtitle(item.getMediaInfo().getSubtitlesTracks().get(1));
		assertFalse(command().contains("-filter_complex"));
	...
	@Test
	void audioRenditionsDoNotBurnVideoSubtitles() throws Exception {
		item.setMediaSubtitle(item.getMediaInfo().getSubtitlesTracks().get(1));
