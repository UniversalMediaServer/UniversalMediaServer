    ...
    return (
      <form onSubmit={groupPermissionsForm.onSubmit(handleGroupPermissionsSubmit)}>
        ...
        <Stack>
          ...
          <Checkbox disabled={group.id < 2} label={i18n.get['ControlDevices']} checked={(permissions & Permissions.devices_control) === Permissions.devices_control} onChange={(event) => event.currentTarget.checked ? addPermission(Permissions.devices_control) : removePermission(Permissions.devices_control)} />
          ...
          <Checkbox disabled={group.id < 2} label={i18n.get['BrowseWebPlayer']} checked={(permissions & Permissions.web_player_browse) === Permissions.web_player_browse} onChange={(event) => event.currentTarget.checked ? addPermission(Permissions.web_player_browse) : removePermission(Permissions.web_player_browse)} />
          <Checkbox disabled={group.id < 2} label={i18n.get['DownloadWebPlayer']} checked={(permissions & Permissions.web_player_download) === Permissions.web_player_download} onChange={(event) => event.currentTarget.checked ? addPermission(Permissions.web_player_download) : removePermission(Permissions.web_player_download)} />
        </Stack>
        {group.id > 1 && (
          <Group position="right" mt="md">
            <Button type="submit">
