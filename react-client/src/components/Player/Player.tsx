...
const Player = ({ i18n, session, sse }: { i18n: I18nInterface, session: SessionInterface, sse: PlayerEventInterface }) => {
  ...
  const hasActionMenu = (playMedia?: PlayMedia) => {
    return data.fullyplayed != null || (playMedia && playMedia.fullyplayed != null) || isVideoMetadataEditable()
  }
  ...
  const getMedia = (media: BaseMedia) => {
    ...
    return (
      <MediaCard
        onLongPress={() => sse.askReqId(media.id, media.goal ? media.goal : 'browse')}
        key={media.id}
        onClick={() => sse.askReqId(media.id, media.goal ? media.goal === 'show' ? 'play' : media.goal : 'browse')}
      >
        {image}
  ...
  const getMediasSelection = (selection: BaseMedia[], title: string) => {
    if (selection && selection.length > 0) {
      ...
      return (
        <>
          <Title order={2} mb="md" size="h4" fw={400}>{i18n.get(title)}</Title>
          <div style={{ height: '240px', overflowY: 'hidden' }} className="media-grid front-page-grid">{medias}</div>
        </>
      )
    }
  }
  ...
  const getMetadataBaseMediaList = (title: string, mediaList?: BaseMedia[]) => {
    if (mediaList && mediaList.length > 0) {
      return (
        <Group gap="xs" mt="sm" style={() => ({ color: computedColorScheme === 'dark' ? 'white' : 'black' })}>
          <Text fw={700}>
            {i18n.get(title)}
            :
            {' '}
  ...
  const getEditMenu = (fullyplayed?: boolean) => {
    return (
  ...
  const getPlayControls = () => {
    if (data.goal === 'show') {
      const playMedia = (data.medias[0]) as PlayMedia
      return (
        <Button.Group>
          ...
          {hasActionMenu(playMedia)
            && getEditMenu(playMedia.fullyplayed)}
          {data.useWebControl && (
    ...
    else if (data.goal === 'browse' && hasActionMenu()) {
      return (
        <Button.Group>
          {' '}
          { getEditMenu(data.fullyplayed) }
          {' '}
        </Button.Group>
      )
    }
