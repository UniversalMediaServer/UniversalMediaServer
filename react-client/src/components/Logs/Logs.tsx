...
const Logs = ({ i18n, session, sse }: { i18n: I18nInterface, session: SessionInterface, sse: ServerEventInterface }) => {
  ...
  useEffect(() => {
    const filterLogLevelFilter = (level: string) => {
      if (logLevelFilter.length === 0 || logLevelFilter.length === 5) {
        return true
      }
    ...
    const filterLogLevel = (logLine: string) => {
      ...
      if (logLevel === 6 && logLevelFilter.length === 0) {
        return true
      }
            ...
            <Stack>
              ...
              <Tooltip label={allowHtml(i18n.get('PackLogConfigurationFileCompressed'))} {...defaultTooltipSettings}>
                <Button
                  ...
                  onClick={() => {
                    setPackerOpened(true)
                    if (traceMode === 0) {
                      setRestartOpened(true)
                    }
                  }}
                >
                  {i18n.get('PackDebugFiles')}
                </Button>
              </Tooltip>
            </Stack>
          </Modal>
          {!fileMode && (
            <>
              <Modal
                ...
                opened={packerOpened}
                onClose={() => {
                  setPackerOpened(false)
                }}
                title={i18n.get('PackDebugFiles')}
              >
                <Button
                  disabled={packerFiles.length === 0}
                  onClick={() => {
                    getPackerZip()
                    setPackerOpened(false)
              ...
              >
                ...
                <Group>
                  <Button
                    onClick={() => {
                      sendAction('Process.Reboot.Trace')
                      setRestartOpened(false)
                      setSettingsOpened(false)
                    }}
                  >
